...
const insertIntoDB = async (
  ...
  await asyncForEach(classSchedules, async (schedule: any) => {
    await OfferedCourseClassScheduleUtils.checkRoomAvaliable(schedule);
    await OfferedCourseClassScheduleUtils.checkFacultyAvaliable(schedule);
  });
  ...
  if (offeredCourseSectionData) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Offered Course Already Exists');
  }

  const createSection = await prisma.$transaction(async transactionClient => {
    const createOfferdCourseSection =
    ...
    const scheduleData = classSchedules.map((schedule: IClassSchedule) => ({
      ...
      offeredCourseSectionId: createOfferdCourseSection.id,
      semesterRegistrationId: isExistOfferedCourse.semesterRegistrationId,
    }));
    ...
    return createOfferdCourseSection;
  });
